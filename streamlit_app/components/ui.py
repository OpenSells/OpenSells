...
def render_whatsapp_fab(phone_e164: str = "+34634159527", default_msg: str = "Necesito ayuda"):
    ...
    """
    # Construir URL wa.me con mensaje codificado
    phone = phone_e164.replace("+", "")
    text = urllib.parse.quote(default_msg)
    wa_url = f"https://wa.me/{phone}?text={text}"

    # CSS + HTML (SVG inline para no depender de assets externos)
    html = f"""
    <style>
      .whatsapp-fab {{
        position: fixed;
      ...
      /* Evitar solaparse con el menú flotante de Streamlit */
      .stActionButton, .st-emotion-cache-1jicfl2 {{ z-index: 1; }}
    </style>
